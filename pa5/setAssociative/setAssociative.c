...
void accessData (
    ...
    /* ... */
    mem_addr_t newtag = addr  >> (s + b);
    // Find the correct cache set
    /* ... */
    int setindex = addr >> b & 0b0011;

    // Cache hit
    /* ... */
    for (int i = 0; i < E; i++) {
        if (cache[setindex][i].valid && cache[setindex][i].tag == newtag) {
            (*hit_count)++;
    ...
    /* ... */
    size_t minimum = cache[setindex][0].recent_use;
    int LRU = 0;
    for (int i = 1; i < E; i++) {
        if (cache[setindex][i].recent_use < minimum) {
            minimum = cache[setindex][i].recent_use;
            LRU = i;
        }
    }
    // If cache set line already in use, then record an eviction
    /* ... */
    if (cache[setindex][LRU].valid) {
        cache[setindex][LRU].tag = newtag;
         cache[setindex][LRU].recent_use = program_counter;
        (*eviction_count)++;
    } else {
        cache_line_t newitem;
        newitem.valid = true;
        newitem.tag = newtag;
        newitem.recent_use = program_counter;
        cache[setindex][LRU] = newitem;
    }
    /* ... */
}
