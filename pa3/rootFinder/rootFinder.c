...
int main(int argc, char *argv[]) {
    ...
    double guess_r_max = x<1.0 ? 1.0 : x; // Think: why is this the maximum guess? 1
    double guess_r_min = 0.0; //0.5
    double guess_r = 0.0; //0.5

    double guess_r_old = guess_r; //0.5
    double error = DBL_MAX; //Big Number

    while ( precision<fabs(error) ) {
        guess_r = ((guess_r_max + guess_r_min)/2); //0.75
        guess_r_old = guess_r; //0.75
        for (int i = 1; i < n; i++) {
            guess_r *=guess_r_old; //0.75^3 = 0.421875
        }
        error = x - guess_r; //0.8665822780873529 - 0.0625
        if (error > 0) {
    ...
    printf("%.8f\n", guess_r_old); // print with default precision
